...
import { Building2, ChevronRight, Plus, StickyNote, Handshake, Sparkles, Database, Upload, Trash2 } from "lucide-react";
...
// Compact org identity card shown at the top of column 2
function OrgCompactHeader({ organization, clientData, onDelete, onEdit }) {
  const [clientStatus, setClientStatus] = React.useState(organization.client_status || "active");
  const [isPushingToSalesforce, setIsPushingToSalesforce] = React.useState(false);
  const [isEnriching, setIsEnriching] = React.useState(false);
  const [enrichingSource, setEnrichingSource] = React.useState(null);

  const initials = organization.organization_name?.charAt(0)?.toUpperCase() || "?";
  const [imgError, setImgError] = React.useState(false);

  const handleStatusToggle = async () => {
    const newStatus = clientStatus === "active" ? "inactive" : "active";
    setClientStatus(newStatus);
    await base44.entities.Organization.update(organization.id, { client_status: newStatus });
    if (onEdit) onEdit();
  };
  ...
  const handlePushToSalesforce = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        toast.success(response.data.message || "Pushed to Salesforce!");
        if (onEdit) onEdit();
      } else {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-3 border-b border-slate-200 bg-gradient-to-r from-blue-600 to-blue-500 flex-shrink-0">
      {/* Logo / initials */}
      <div className="w-10 h-10 rounded-full flex-shrink-0 border-2 border-white/30 overflow-hidden bg-white/20 flex items-center justify-center">
        {organization.logo_url && !imgError
          ? <img src={organization.logo_url} alt={organization.organization_name} className="w-full h-full object-cover" onError={() => setImgError(true)} />
      ...
      <div className="flex-1 min-w-0">
        <p className="text-white font-semibold text-sm leading-tight truncate">{organization.organization_name}</p>
        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
          {organization.organization_type && (
            <Badge className="bg-white/20 text-white border-0 text-xs px-1.5 py-0">{organization.organization_type}</Badge>
          )}
          {organization.state && (
            <span className="text-white/70 text-xs">{organization.state}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <div
          style={{ backgroundColor: clientStatus === "active" ? "hsl(142, 76%, 36%)" : "rgba(255,255,255,0.3)" }}
          className="relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors"
          onClick={handleStatusToggle}
          title={clientStatus === "active" ? "Active" : "Inactive"}
        >
          <div className={`pointer-events-none block h-4 w-4 rounded-full bg-white shadow ring-0 transition-transform ${clientStatus === "active" ? "translate-x-4" : "translate-x-0"}`} />
        </div>

        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-white hover:bg-white/20" title="Smart Enrich">
          <Sparkles className="w-3.5 h-3.5" />
        </Button>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" disabled={!organization.ein} className="h-7 w-7 p-0 text-white hover:bg-white/20" title="Enrich from source">
              <Database className="w-3.5 h-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem>CharityAPI</DropdownMenuItem>
            <DropdownMenuItem>ProPublica</DropdownMenuItem>
            <DropdownMenuItem>Nonprofit Check Plus</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>

        <Button variant="ghost" size="sm" onClick={handlePushToSalesforce} disabled={isPushingToSalesforce} className="h-7 w-7 p-0 text-white hover:bg-white/20" title="Push to Salesforce">
