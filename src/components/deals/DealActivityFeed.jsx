...
export default function DealActivityFeed({ deal, lifecycleStages = [] }) {
  ...
  const { data: interactions = [] } = useQuery({
    queryKey: ["deal-interactions", deal.id],
    queryFn: () => base44.entities.Interaction.filter({ organization_id: deal.organization_id, contact_id: deal.id }, "-created_date"),
  });
  ...
  const { data: activityRecords = [] } = useQuery({
    queryKey: ["deal-activity-records", deal.id],
    queryFn: () => base44.entities.Activity.filter({ organization_id: deal.organization_id, contact_id: deal.id }, "-created_date"),
  });

  const mergedActivity = [
    ...(deal.created_date
      ? [{ ...deal, id: `deal-created-${deal.id}`, type: "deal", timestamp: deal.created_date, isCreatedEvent: true }]
      : []),
                            ...
                            {item.type === "note" ? (
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-32">
                                  <DropdownMenuItem onSelect={() => setEditingNote(item)}>
