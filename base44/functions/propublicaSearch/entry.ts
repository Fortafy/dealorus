...
Deno.serve(async (req) => {
  try {
    ...
    if (!response.ok) {
      ...
      return Response.json({ error: 'Failed to search ProPublica', status: response.status }, { status: response.status });
    }
    ...
    const maxPages = Math.min(result.num_pages || 1, 20);
    ...
    return Response.json({ error: error.message || 'Failed to fetch from ProPublica' }, { status: 500 });
  }
});